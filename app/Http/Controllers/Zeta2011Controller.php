...
class Zeta2011Controller
{
    ...
    that,given a matrix A consisting of N rows and M columns describing a board of ball switches and a non-negative number K of balls inserted into this board,
    returns the number of balls that will leave the board through the bottom edge of the bottom-right switch.
    ...
    public function ball_switch_board()
    {
        function detectNextDirection($direction, $matrixItem) {
            return $matrixItem ?: $direction;
        }
        ...
        function makeNextTurn(&$row, &$col, $toDown) {
            if ($toDown) {
                $row++;
            } else {
                $col++;
        ...
        function solution($matrix, $balls) {
            $DOWN = -1;

            $matrixRows = count($matrix);
            $matrixCols = $matrixRows ? count($matrix[0]) : 0;
            $lastCol = $matrixCols - 1;

            $countOfBallsNeeded = 0;
            for ($i = 0; $i < $balls; $i++) {
                $direction = $DOWN;
                for ($row = 0, $col = 0; $row < $matrixRows && $col < $matrixCols;) {
                    $direction = detectNextDirection($direction, $matrix[$row][$col]);

                    switchItem($matrix[$row][$col]);

                    makeNextTurn($row, $col, $direction == $DOWN);
                }

                if ($direction == $DOWN && $col == $lastCol) {
                    $countOfBallsNeeded++;
                }
            }

            return $countOfBallsNeeded;
        }
